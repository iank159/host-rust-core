...
`TrUApiTransport.codecVersion`, `CreateTransportOptions` and
...
`createTransport(provider)` takes no options argument, and `createClient`
takes a `TrUApiTransport` (every value that satisfied
